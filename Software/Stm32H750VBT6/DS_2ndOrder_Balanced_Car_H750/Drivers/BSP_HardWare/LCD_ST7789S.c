...
void TFTSPI_Init(void) {
    ...
    {
					/***ST7735S方向控制指令***/
        case 0:TFTSPI_Write_Byte(0x00);
        break;
        case 1:TFTSPI_Write_Byte(0xC0);
        break;
 ...
 * @note     无
 *
 * @see      TFTSPI_Write_halfWord(0xFFFF);
 *
 * @date     2019/6/13 星期四
 */
void TFTSPI_Write_halfWord(unsigned short dat)
{
		uint8_t Data[2] = {0,0};
		TFTSPI_CS_L;//片选
    TFTSPI_DC_H;       //DC=1发送数据
...
void TFTSPI_Set_Pos(unsigned short xs, unsigned short ys, unsigned short xe,
        unsigned short ye) 
{
	/***不同分辨率屏幕的坐标设置需要不同的坐标偏移量***/
	if(USE_HORIZONTAL==0)
	{
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(xs);		//行起始坐标
		TFTSPI_Write_halfWord(xe);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(ys);		//列起始坐标
		TFTSPI_Write_halfWord(ye);		//列终止坐标
    ...
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		...
		TFTSPI_Write_halfWord(xe);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(ys+80);		//列起始坐标
		TFTSPI_Write_halfWord(ye+80);		//列终止坐标
    TFTSPI_Write_Cmd(0x2C);       //GRAM接收MCU数据或命令
	}
	...
	{
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(xs+80);		//行起始坐标
		TFTSPI_Write_halfWord(xe+80);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(ys);		//列起始坐标
		TFTSPI_Write_halfWord(ye);		//列终止坐标
    TFTSPI_Write_Cmd(0x2C);       //GRAM接收MCU数据或命令
...
void TFTSPI_Addr_Rst(void) {
    ...
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(0);		//行起始坐标
		TFTSPI_Write_halfWord(240-1);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(0);		//列起始坐标
		TFTSPI_Write_halfWord(240-1);		//列终止坐标
    TFTSPI_Write_Cmd(0x2C);       //GRAM接收MCU数据或命令
	}
	else if(USE_HORIZONTAL==1)
	{
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		...
		TFTSPI_Write_halfWord(240-1);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(0+80);		//列起始坐标
		TFTSPI_Write_halfWord(240-1+80);		//列终止坐标
    TFTSPI_Write_Cmd(0x2C);       //GRAM接收MCU数据或命令
	...
	{
    TFTSPI_Write_Cmd(0x2A);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(0+80);		//行起始坐标
		TFTSPI_Write_halfWord(240-1+80);		//行终止坐标
    TFTSPI_Write_Cmd(0x2B);       //Colulm addRSTs set
		TFTSPI_Write_halfWord(0);		//列起始坐标
		TFTSPI_Write_halfWord(240-1);		//列终止坐标
    TFTSPI_Write_Cmd(0x2C);       //GRAM接收MCU数据或命令
...
//    for (j = 0; j < (ye - ys + 1); j++) {
//        for (i = 0; i < (xe - xs + 1); i++) {
//            TFTSPI_Write_halfWord(color_dat);
//        }
//    }
	#endif
	...
	switch(Size)
	{
		case fsize_6X8:
			...
			for (j = 0; j < 8; j++) {
					for (i = 0; i < 6; i++) {
							if ((Font_code8X6[Char - 32][i]) & (0x01 << j))
									TFTSPI_Write_halfWord(word_color);
							else
									TFTSPI_Write_halfWord(back_color);
					}
			}			
			break;
